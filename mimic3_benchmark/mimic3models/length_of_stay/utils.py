    ...
    def _generator(self):
        ...
        while True:
            ...
            while remaining > 0:
                ...
                (Xs, ys, ts, names, stay_ids) = common_utils.sort_and_shuffle([Xs, ys, ts, names, stay_ids], B)
                ...
                for i in range(0, current_size, B):
                    ...
                    batch_ts = ts[i:i+B]
                    batch_stay_ids = stay_ids[i:i+B]   # ← NEW
                    ...
                    if not self.return_names:
                        ...
                        yield {"data": batch_data, "names": batch_names, "ts": batch_ts, "stay_id": batch_stay_ids}
...
def save_results(names, stay_ids, ts, pred, y_true, path):
    common_utils.create_directory(os.path.dirname(path))
    with open(path, 'w') as f:
        f.write("stay,stay_id,period_length,prediction,y_true\n")
        for (name, sid, t, x, y) in zip(names, stay_ids, ts, pred, y_true):
            f.write("{},{},{:.6f},{:.6f},{:.6f}\n".format(name, sid, t, x, y))